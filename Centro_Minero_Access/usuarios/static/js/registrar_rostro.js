// Envolver TODO en DOMContentLoaded para asegurar que los elementos existan
document.addEventListener('DOMContentLoaded', () => {
    // DOM elements (igual que original)
    ...
    const cancelBtn = document.getElementById('cancelBtn');  // NUEVO: Para el botón cancelar

    let stream = null;
    let capturedImage = null;

    // Función showStatus (igual)
    ...
    // NUEVO: Handler para botón cancelar (solo si existe el botón, i.e., usuario inactivo)
    if (cancelBtn) {
        cancelBtn.addEventListener('click', () => {
            if (confirm('¿Seguro? El usuario quedará inactivo y no se mostrará en la lista.')) {
                showStatus('Registro cancelado. Usuario inactivo.', 'info');
                setTimeout(() => { window.location.href = listarUsuariosUrl; }, 1500);
            }
        });
    }
    ...
    // send to backend (igual, pero asegúrate de que usuarioId y procesarRostroUrl estén definidos en HTML)
    ...
    // cleanup on unload (igual, pero muévelo fuera del DOMContentLoaded si quieres, o déjalo aquí)
