...
def reconocer_rostro(request):
    try:
        ...
        # Buscar usuarios con rostro registrado
        ...
        if user_id:
            ...
            # 🔑 Vincular con Django User para manejar login
            auth_user, created = AuthUser.objects.get_or_create(
                username=usuario.carnet,
                defaults={"first_name": usuario.nombre}
            )
            ...
            return JsonResponse({
                ...
                'confianza': round(confidence * 100, 1) if confidence else 0,
                'acceso_autorizado': True,
                'login': True   # 👉 frontend sabrá que inició sesión
            })
        else:
            return JsonResponse({
                'success': False,
