...
async function capturarRostro() {
    if (!cameraActive || !stream) {
        agregarLog("⚠️ Activa la cámara primero");
        return;
    }
    ...
    try {
        ...
        const response = await fetch("/usuarios/api/reconocer-rostro/", {
            method: "POST",
            headers: {
        ...
        if (data.success && data.acceso_autorizado) {
            permitirAcceso(
                { nombre: data.usuario.nombre, tipo: data.usuario.tipo },
                data.usuario.carnet,
                "Reconocimiento Facial"
            );
            agregarLog(`✅ Reconocido: ${data.usuario.nombre} (${data.confianza}%)`);

            // 🔑 Si el backend también inició sesión, refrescamos la página
            if (data.login) {
                setTimeout(() => {
            ...
            denegarAcceso("Rostro no reconocido", "----", "Reconocimiento Facial");
            agregarLog("❌ Rostro no reconocido");
        }

    } catch (error) {
        console.error("Error en reconocimiento:", error);
        agregarLog("❌ Error al procesar reconocimiento facial");
    }
}
